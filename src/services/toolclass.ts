...
function extractErrorMessage(error: any): string {
  ...
  if (statusCode === 403) {
    return "API key invalid or required API not enabled. Check: console.cloud.google.com → APIs & Services → Enable the relevant API";
  }
  if (statusCode === 429) {
...
export class GoogleMapsTools {
  ...
  async getAirQuality(
    ...
    try {
      ...
      if (includeHealthRecommendations) extraComputations.push("HEALTH_RECOMMENDATIONS");
      if (includePollutants) extraComputations.push("POLLUTANT_CONCENTRATION");

      const body: any = { location: { latitude, longitude } };
      if (extraComputations.length > 0) body.extraComputations = extraComputations;
      ...
      if (data.healthRecommendations) result.healthRecommendations = data.healthRecommendations;
  ...
  async getStaticMap(params: {
    ...
    try {
      ...
      if (params.center) queryParts.push(`center=${encodeURIComponent(params.center)}`);
      if (params.zoom !== undefined) queryParts.push(`zoom=${params.zoom}`);
      if (params.markers) {
      ...
      return { base64, size: buffer.length, dimensions };
    } catch (error: any) {
