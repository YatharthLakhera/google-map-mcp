...
export class NewPlacesService {
  ...
  async searchText(params: {
    ...
    try {
      ...
      if (params.minRating) request.minRating = params.minRating;
  ...
  async getPlaceDetails(placeId: string) {
    try {
      ...
      const [place] = await this.client.getPlace(
        ...
        {
          otherArgs: {
            headers: { "X-Goog-FieldMask": this.placeFieldMask },
          },
        }
      );
  ...
  private isCurrentlyOpen(openingHours: any, utcOffsetMinutes?: number, currentOpeningHours?: any): boolean {
    if (typeof currentOpeningHours?.openNow === "boolean") return currentOpeningHours.openNow;
    ...
    for (const period of periods) {
      ...
      if (openDay === undefined || openMinutes === undefined) continue;
      ...
      if (end <= start) end += minutesInWeek;

      let comparableLocalTime = localTimeValue;
      while (comparableLocalTime < start) comparableLocalTime += minutesInWeek;

      if (comparableLocalTime >= start && comparableLocalTime < end) return true;
    }
  ...
  private getLocalTimeComponents(utcOffsetMinutes?: number): { day: number; minutes: number } {
    ...
    if (typeof utcOffsetMinutes === "number" && Number.isFinite(utcOffsetMinutes)) {
      ...
      return { day: localTime.getUTCDay(), minutes: localTime.getUTCHours() * 60 + localTime.getUTCMinutes() };
    }
