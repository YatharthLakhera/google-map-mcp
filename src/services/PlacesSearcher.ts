...
import { NewPlacesService } from "./NewPlacesService";
import { RoutesService, parseDuration, formatDistance, formatDuration } from "./RoutesService";
...
export class PlacesSearcher {
  ...
  async searchNearby(params: {
    ...
    openNow?: boolean;
    minRating?: number;
  }): Promise<{ success: boolean; error?: string; data?: any[]; location?: any }> {
    try {
  ...
  async geocode(address: string): Promise<{ success: boolean; error?: string; data?: { location: { lat: number; lng: number }; formatted_address: string; place_id: string } }> {
    try {
  ...
  async reverseGeocode(latitude: number, longitude: number): Promise<{ success: boolean; error?: string; data?: { formatted_address: string; place_id: string; address_components: any[] } }> {
    try {
  ...
  ): Promise<{ success: boolean; error?: string; data?: { distances: any[][]; durations: any[][]; origin_addresses: string[]; destination_addresses: string[] } }> {
    try {
  ...
  async getDirections(
    ...
    try {
      const result = await this.routesService.computeRoutes({
        ...
        mode,
        ...(departure_time ? { departureTime: new Date(departure_time) } : {}),
        ...(arrival_time ? { arrivalTime: new Date(arrival_time) } : {}),
      });
      return { success: true, data: result };
    } catch (error) {
      return {
  ...
  async getStaticMap(params: {
    ...
    markers?: string[];
    path?: string[];
  }): Promise<{ success: boolean; error?: string; data?: { base64: string; size: number; dimensions: string } }> {
    try {
  ...
  ): Promise<{ success: boolean; error?: string; data?: Array<{ elevation: number; location: { lat: number; lng: number } }> }> {
    try {
  ...
  async planRoute(params: {
    ...
    const optimizedIndexes = routeResult.optimizedIntermediateWaypointIndex;
    if (
      shouldOptimize &&
      ...
      const reordered = optimizedIndexes
        .map((i: number) => intermediateGeocoded[i])
        .filter((s): s is typeof geocoded[0] => s !== undefined);
      // Only use optimized order if every intermediate was mapped successfully
      if (reordered.length === intermediateGeocoded.length) {
        orderedStops = [geocoded[0], ...reordered, geocoded[geocoded.length - 1]];
      } else {
  ...
  async localRankTracker(params: {
    ...
    try {
      ...
      try {
        ...
        if (details.success && details.data) targetName = details.data.name;
      } catch {
        // ignore
      }
  ...
  private async scanKeywordGrid(
    ...
    return {
      metrics: { arp, atrp, solv, found_in: `${rankedPoints.length}/${totalPoints}` },
      grid: gridResults,
    };
  }
}
